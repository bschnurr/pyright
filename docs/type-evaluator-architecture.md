...
- In progress:
  - `evaluatorCore.ts` extraction: return-type-inference context stack management, symbol resolution stack operations, `getLineNumForNode`, `getAliasFromImportNode`, `isTypeFormSupportedForNode`, `isFinalVariableDecl`, `includesVariableTypeDeclCheck`, `isLegalTypeAliasExprForm`, `isPossibleTypeAliasDeclCheck`, `getIndexAccessMagicMethodNameForUsage`, `convertArgumentNodeToArg`, `getFunctionFullNameFromNode`, `getPseudoGenericTypeVarNameForParam`, `getSpeculativeNodeForCallSite`.
