...
function hasMissingLeafAndSequenceMembers(
    ...
    if (!elements.length || !elements.every(isFunctionOrOverloaded)) {
        return false;
    }
    ...
    return elements.every((elementType) => {
        const functionClass = evaluator.getFunctionClassType(elementType);
        return (
            functionClass &&
            functionClass.shared.mro.every(isInstantiableClass) &&
            !lookUpClassMember(functionClass, memberName) &&
            !lookUpClassMember(functionClass, '__getitem__')
        );
    });
}
